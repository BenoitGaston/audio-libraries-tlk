...
from convertion_functions import m3u8_to_csv,df_to_m3u8,PL_to_m3u8,convert_playlists
...
def convert_lib_to_csv(path_to_lib_folder,lib_name,path_to_dest_folder):
    ...
    df[all_cols].to_csv(os.path.join(path_to_dest_folder,f"{lib_name.split('.')[0]}.csv"))
...
def get_lib_as_csv(path_to_lib_folder,lib_name,path_to_dest_folder):
    ...
    df_lib.loc[:,'CloudLocation'] = 'Cloud/'+df_lib.loc[:,'Album Artist']+'/'+df_lib.loc[:,'Album']
    df_lib.loc[:,'Location'] = df_lib.loc[:,'Location'].fillna(df_lib.loc[:,'CloudLocation'])
...
def group_by_cd(x):
    ...
    return pd.Series({'CD Total Time': total_duration,
                       'CD Size' : size,
                       'CD Scoring' :scoring ,
...
def get_cd_df(df_lib,
    ...
    df_lib.loc[:,'Album Artist'] =  df_lib['Album Artist'].apply(lambda x: str(x).strip())
...
def add_grand_genre(df_lib):
    ...
    grand_genre = ["Air d'Opéra","Chant","Classique","Électro",
...
class LibScan:
    ...
    def get_locations_with_multiple_album(self):
        ...
        grouped_df = df.groupby(by='Album Location').apply(group_by_location).reset_index()

        df = pd.merge(df, grouped_df, on='Album Location',how = 'left')

        grouped_df[grouped_df['Number of Album in Location']>1].to_csv(os.path.join(self.path_to_dest_folder,
    ...
    def split_lib_by_features(self,
        ...
        if not mix_genre:
            ...
            for genre in self.df_cd[split_genre].sort_values().unique():
                ...
                list_of_df_to_split.append(self.df_cd[self.df_cd[split_genre] == genre].copy())
        else:
        ...
        for df_cd_to_split in list_of_df_to_split:
            ...
            sorted_df.loc[:,'PL Id'] = np.cumsum((sorted_df.loc[:,maximum_size_feature])
                                                /(maximum_bin_size)).astype(int)+ max_pl_id
        ...
        all_sorted_df.loc[:,'PL Name'] = all_sorted_df.loc[:,'PL Name'] +all_sorted_df.loc[:,'PL Id']
        ...
        lib_df = lib_df.merge(all_sorted_df[group_features+['PL Name']], on=group_features)

        for pl_name in all_sorted_df.loc[:,'PL Name'].unique():

            lib_df[lib_df.loc[:,'PL Name']== pl_name].to_csv(os.path.join(path_to_pl, f'{pl_name}.csv'))
                                                            
            df_to_m3u8(lib_df[lib_df.loc[:,'PL Name']== pl_name],
                        pl_name, 
                        path_to_pl)
    ...
    def split_lib_by_location(self,
        ...
        lib_df.loc[:,'audio_loc_in_loc'] = lib_df.Location_rep.apply(lambda x : audio_location in x)


        lib_df.loc[:,'level_max'] = np.where(lib_df.loc[:,'audio_loc_in_loc'],
            lib_df.Location_rep.apply(lambda x : len(x.split(audio_location)[-1].split('/'))),
                                                            0)

        lib_df.loc[:,'pl_name'] = np.where(lib_df.loc[:,'audio_loc_in_loc'],
            lib_df.Location_rep.apply(lambda x : '_'.join(x.split(audio_location)[-1].split('/')[0:min([level+1,len(x.split(audio_location)[-1].split('/'))
                                                                                    ])
                                                                                    ])),
                                                            0)
        path_to_pl = os.path.join(self.path_to_dest_folder,audio_location.replace('/','_' )+'_PL')

        if audio_location.replace('/','_' )+'_PL' not in os.listdir(self.path_to_dest_folder):
                os.mkdir(path_to_pl)
        ...
        for pl_name in lib_df.loc[:,'pl_name'].unique():
            if pl_name!=0:
                ...
                if len(lib_df[lib_df.pl_name == pl_name])>1:
                    df_to_m3u8(lib_df[lib_df.pl_name == pl_name],
                            pl_name.replace(' ','_'), 
                            path_to_pl)
