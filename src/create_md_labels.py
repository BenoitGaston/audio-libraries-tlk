...
def built_dict_of_styles(theme):
    ...
    base_3 = (";line-height:1.25;font-family:sans-serif;-inkscape-font-specification:'sans-serif, Bold';"+
    ...
    i_style_2 = f"{base_2}2.11667px{base_3}0.264583;fill:#{main_text_color};fill-opacity:1"
    ...
    i_style_8 = (f"{base_2}2.11667px{base_3};text-align:center;letter-spacing:0px;word-spacing:0px;writing-mode:lr-tb;text-anchor:middle;"+
                 f"fill:#{main_text_color};fill-opacity:1;stroke:none;stroke-width:0.264583")
    i_style_9 = (f"{base_2}2.11667px;font-family:sans-serif;-inkscape-font-specification:'sans-serif, "+
    ...
    ds_style_1 = (f"{base_2}2.11667px;line-height:1.25;font-family:sans-serif;-inkscape-font-specification:'sans-serif, Bold';font-variant-ligatures:normal;"+
    ...
    return {'i_styles' : [i_style_1,i_style_2,i_style_3,i_style_4,i_style_5,i_style_6,i_style_7,i_style_8,i_style_9],
            'ds_styles' : [ds_style_1,ds_style_2],
            'od_styles' : [od_style_1,od_style_2,od_style_3,od_style_4,od_style_5,od_style_6],
            'o_styles' : [o_style_1,o_style_2,o_style_3,o_style_4,o_style_5],
            'id_styles' : [id_style_1,id_style_2,id_style_3,id_style_4,id_style_5,id_style_6],}
...
def update_temp_svgs(substitution_dict,path_to_temp_files_folder):
    ...
    open(path_to_temp_files_folder/f'temp-0.svg', 'w').write(open(path_to_temp_files_folder/f'temp-1.svg').read())
    i=1
    

    for key in substitution_dict.keys():

        open(path_to_temp_files_folder/f'temp-{i%2}.svg', 'w').write(open(path_to_temp_files_folder/f'temp-{(i+1)%2}.svg').read().replace(key,
                                                                                  substitution_dict[key]))
        i+=1
    open(path_to_temp_files_folder/f'temp-{(i)%2}.svg', 'w').write(open(path_to_temp_files_folder/f'temp-{(i+1)%2}.svg').read())
...
def get_theme(row):
    ...
    for theme_item in ['background_color','md_logo_background_color']:
        if is_valid_hexa_code(str(row['background_color'])):
            ...
            theme[theme_item] = param.color_map.get(row['background_color'],param.background_default)
                                              
    for theme_item in ['main_text_color','md_logo_text_color','triangle_color','insert_color']:
        if is_valid_hexa_code(str(row['text_color'])):
            theme[theme_item] = row['text_color']
        else:
            theme[theme_item] = param.color_map.get(row['text_color'],param.text_default)

    return   theme                                     

def create_substitucion_dict(one_page_md_labels_df,
                             image_dict):
    ...
    for md_id,row in one_page_md_labels_df.iterrows():
        ...
        substitution_dict = substitution_dict | {#f">id_artist_{md_id}-1<" :  f">{row['Album Artist']}<", #line 1
        ...
        if str(row['Album Year']) == 'nan':
            substitution_dict = substitution_dict | {f">i_year_{md_id}<" :  f"> <",
                                                     f">o_album_{md_id}-year_{md_id}<" :  f">{row['Display Album']}<"}
        else:
            substitution_dict = substitution_dict | {f">i_year_{md_id}<" :  f">{int(row['Album Year'])}<",
        ...
        if str(row['Display Album Artist']) == 'nan':
            ...
            substitution_dict = substitution_dict | {f">i_artist_{md_id}<" :  f">{row['Display Album Artist']}<"}
  
        substitution_dict = substitution_dict |{ f"i_style_{md_id}-{style_id}" : dict_of_styles["i_styles"][style_id-1] for style_id in range(1,10)}
        substitution_dict = substitution_dict |{ f"id_style_{style_id}-{md_id}" : dict_of_styles["id_styles"][style_id-1] for style_id in range(1,7)}
        substitution_dict = substitution_dict |{ f"o_style_{md_id}-{style_id}" : dict_of_styles["o_styles"][style_id-1] for style_id in range(1,6)}
        substitution_dict = substitution_dict |{ f"od_style_{style_id}-{md_id}" : dict_of_styles["od_styles"][style_id-1] for style_id in range(1,7)}
        substitution_dict = substitution_dict |{ f"sd_style_{md_id}-{style_id}" : dict_of_styles["ds_styles"][style_id-1] for style_id in range(1,3)}
        substitution_dict = substitution_dict |{ f"s_style_{md_id}-{style_id}" : dict_of_styles["ds_styles"][style_id-1] for style_id in range(1,3)}
        
        #if str(row['album_2']) == 'nan':
        if f"{row['Album Artist']}-{row['Album']}" in image_dict.keys():
            substitution_dict[f"image_number_{md_id}-"] =  image_dict[f"{row['Album Artist']}-{row['Album']}"]
        ...
        substitution_dict[f">sd_artist_album_{md_id}<"] = f">{row['Display Album Artist']} - {row['Display Album']}<"
...
class MiniDiscCovers():
    ...
    def create_mini_disc_df(self):
        ...
        mini_disc_playlists = [file for file in os.listdir(self.path_to_music_folder)
                                    if 'minidisc' in file.lower().replace(' ','').replace('_','').replace('-','')
                                    and file.endswith('.m3u8')]
        
        if len(mini_disc_playlists) ==0:
            print(f'No minidisc playlist in location {self.path_to_music_folder}.')
            print(f'To run this script, you need a m3u8 playlist with "minidic" in the filename.')
            return pd.DataFrame([]),pd.DataFrame([])
        
        mini_disc_playlist = mini_disc_playlists[0]

        if f'{mini_disc_playlist.replace('.m3u8','')}_songs.csv' in os.listdir(self.path_to_music_folder):
            mini_disc_songs_df = pd.read_csv(self.path_to_music_folder/f'{mini_disc_playlist.replace('.m3u8','')}_songs.csv')
        else:


            mini_disc_songs_df = m3u8_to_csv(path_to_playlist =self.path_to_music_folder, 
                                            playlist_name = mini_disc_playlist)
            os.remove(self.path_to_music_folder / mini_disc_playlist.replace("m3u8", "csv"))
            ...
            mini_disc_songs_df.to_csv(self.path_to_music_folder/f'{mini_disc_playlist.replace('.m3u8','')}_songs.csv')
        
        if f'{mini_disc_playlist.replace('.m3u8','')}_albums.csv' in os.listdir(self.path_to_music_folder):
            mini_disc_album_df = pd.read_csv(self.path_to_music_folder/f'{mini_disc_playlist.replace('.m3u8','')}_albums.csv')
        else:    

            library_processing = LibraryProcessing(df_lib = mini_disc_songs_df,
                                                path_to_playlist_folder = self.path_to_dest_folder)

            mini_disc_album_df = library_processing.get_grouped_df(disc_or_album="Album", path_to_destination = self.path_to_dest_folder)
            os.remove(self.path_to_dest_folder/"Album.csv")
            mini_disc_album_df.loc[:,["Album","Album Artist"]] = mini_disc_album_df.loc[:,["Album","Album Artist"]].replace('/','-', regex=True).replace(':','-', regex=True)

            mini_disc_album_df.loc[:,'Display Album'] = mini_disc_album_df.loc[:,'Album']
            mini_disc_album_df.loc[:,'Display Album Artist'] = mini_disc_album_df.loc[:,'Album Artist'] 
            mini_disc_album_df.loc[:,'text_color'] = None
            mini_disc_album_df.loc[:,'background_color'] = None
            mini_disc_album_df.loc[:,'text_color'] = None

            odered_columns = ["Album","Album Artist", 'Display Album','Display Album Artist',
                            'background_color','text_color',"Album Year",
                        "Album Total Time","Album Genre" 
                        ,"Num Tracks","Album Has Multiple Locations", 
                        "Album Has Multiple Genre","Album Location",]
            
            mini_disc_album_df = mini_disc_album_df[odered_columns]
            mini_disc_album_df.to_csv(self.path_to_music_folder/f'{mini_disc_playlist.replace('.m3u8','')}_albums.csv')
    ...
    def copy_covers_to_dest_path(self):
        ...
        for _,row in self.mini_disc_album_df.iterrows():
            ...
            cover_files = [ f for f in images_files 
                           if (str(f.name).startswith('cover.') or
                               str(f.name).startswith(f'{album_title}.') )
                           ]

            if len(cover_files) == 0:

                edit_cover_artwork = EditCoverArtwork(self.mini_disc_songs_df[self.mini_disc_songs_df.Album ==album_title],
            ...
            if len(cover_files)==0:
                ...
                shutil.copy2(cover_file,
                            self.path_to_dest_folder/f'{album_artist}-{album_title}.{cover_file.name.split(".")[-1]}'
                            )


    def loop_over_images_files(self,):
        list_of_images_dict = []
        ...
        for full_file_name in images_files:
            ...
            try:
                ...
                logging.warning(f' File {full_file_name} is not respecting the format Album - Artist.jpg')
    ...
    def get_converted_images_dict(self):
        ...
        for _,row in self.mini_disc_album_df.iterrows():
            ...
            cover_file_1 = [ f for f in images_files 
                            if (f.name.startswith(f'{artist}-{album}.') )]
            
            if len(cover_file_1)==0:
                logging.warning(f" ! Warning no cover file found for {artist}-{album} !")
                break
    ...
    def build_album_labels(self,
        ...
        #list_num_pages = []#,32,8]
        template_names = [f'MD-Labels-Inner-Template.svg',
        ...
        for i in range(len(list_num_pages)):
            ...
            total_number_of_pages = int(len(self.mini_disc_album_df)/number_of_labels_per_page)

            for page_id in range(0,total_number_of_pages+1):
                
                one_page_md_labels_df = self.mini_disc_album_df.iloc[(page_id 
                                                        * number_of_labels_per_page): np.min([(page_id+1) 
                                                                                                * number_of_labels_per_page,
                                                                                                len(self.mini_disc_album_df) ])]
                
                one_page_md_labels_df = pd.concat([one_page_md_labels_df, 
                                                   pd.DataFrame({col: ['0']*(number_of_labels_per_page-len(one_page_md_labels_df))
                                                                  for col in one_page_md_labels_df.columns})])

                substitution_dict = create_substitucion_dict(one_page_md_labels_df,self.image_dict)

                open(self.path_to_dest_folder/'temp-1.svg', 'w').write(open(path_to_templates/template_name).read())
                open(self.path_to_dest_folder/'temp-0.svg', 'w').write(open(path_to_templates/template_name).read())
                
                update_temp_svgs(substitution_dict,path_to_temp_files_folder =self.path_to_dest_folder )

                open(self.path_to_dest_folder/f"{template_name.replace('.svg','').replace('-Template','')}-page-{page_id+1}.svg", 'w').write(open(self.path_to_dest_folder/'temp-1.svg').read())
    ...
    def build_md_labels(self):
        ...
        if len(self.mini_disc_album_df) ==0:
            ...
            logging.warning(f" ! Warning no cover file found for mini_disc_album_df is empty !")
