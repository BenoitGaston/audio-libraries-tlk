...
class iTunesLibraryScan:
    ...
    def convert_lib_to_csv(self):
        ...
        for i, _ in enumerate(root[0][17]):
            ...
            if i % 2 == 1:
                for j, _ in enumerate(root[0][17][i]):
                    if root[0][17][i][j].tag == "key":
                        if root[0][17][i][j].text in param.itunes_bool_columns:
        ...
        df[param.itunes_usefull_cols].to_csv(
            self.path_to_dest_folder / self.lib_name.replace(".xml", ".csv"),
            index=False,
        )

        return df[param.itunes_usefull_cols]
    ...
    def get_itunes_library_as_csv(self):
        ...
        if csv_name in os.listdir(self.path_to_dest_folder):
            """dtype_dict = {col:str for col in param.itunes_usefull_cols if col in param.itunes_bool_columns}
            dtype_dict.update({col:float for col in param.itunes_usefull_cols if col in param.itunes_numeric_columns })
